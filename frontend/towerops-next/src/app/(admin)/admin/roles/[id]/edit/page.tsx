...
import { useRole, useRolePermissions, useUpdateRole } from '@/hooks/use-roles';
...
export default function EditRolePage() {
  ...
  useEffect(() => {
    ...
    setDisplayName(roleQuery.data.displayName);
  ...
  if (roleQuery.isLoading || permissionsQuery.isLoading) {
    return <LoadingState label="Loading role editor..." />;
  }

  if (roleQuery.isError || permissionsQuery.isError || !roleQuery.data || !permissionsQuery.data) {
    return <ErrorState message="Failed to load role editor." onRetry={() => roleQuery.refetch()} />;
  }
  ...
  return (
    <main className="space-y-6 p-6">
      <div>
        ...
        <p className="text-sm text-slate-400">Update role metadata and permission assignments.</p>
      </div>

      <section className="text-sm">
        <Link className="text-brand-blue underline" href={`/admin/roles/${roleId}`}>
          Back to role detail
        </Link>
      </section>
      ...
      <Button
        ...
        onClick={async () => {
          try {
            ...
            setFeedback({ tone: 'success', message: 'Role updated successfully. Redirecting...' });
